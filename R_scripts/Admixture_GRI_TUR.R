...
ggplot(data = admix, aes(x = K, y = CV)) +
  geom_line() +
  labs(title = "Cross-validation error: GRI & TUR") +
  scale_x_continuous(breaks = seq(1, 12, by = 1))+
  theme_minimal()

# Load table.
tbl=read.table("pca_075_gri_tur.2_1.Q")

# Load sample set.
samples=read.csv('spp_loc_GRI_TUR.csv')
...
tbl3 <- tbl2 %>% group_by(ID) %>% mutate(
  ...
  arrange(match(likely_assignment, c(2, 1))) %>%
  ungroup() %>% 
...
plot1 <- tbl4 %>% ggplot() +
  ...
  geom_col(aes(x = ID, y = q, fill = cluster),
